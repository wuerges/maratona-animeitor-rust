...
- `animeitor-client/` — Leptos 0.8 CSR (wasm) UI, built with `trunk` from that dir (`make rebuild-client-for-release`, dev: `trunk serve`). Nightly toolchain pin lives in `animeitor-client/rust-toolchain.toml`.
...
- `server/` crates — `data` (shared types, wasm-safe), `service`, `server-v2`, `cli` (binaries incl. `animeitor-server`). Build: `cargo build -p cli`, release: `make rebuild-server-for-release`.
