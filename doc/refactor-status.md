...
- **Deploy**: Dockerfile com `--public-url /animeitor/` e binário do feeder na imagem; serviço `feeder` no compose; `.env` sem `SECRET`/`SEDES` e com `SERVER_URL`; Makefile com mount raiz (landing) e `run-config` via feeder; naquadah.Makefile, docker-compose.regional-exemplo.yaml, README raiz e server/README atualizados.
