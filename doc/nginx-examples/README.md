...
We need to set this for both the `animeitor-server` server and NGINX.
...
For the `animeitor-server` animeitor server, we can add an entry in `/etc/security/limits.conf`:
...
and only use the `animeitor-server` server for the api.

Another way is to host http using the `animeitor-server` server, and https using NGINX as a reverse proxy.
...
data and forward the api requests to the animeitor-server server handling only the api.
