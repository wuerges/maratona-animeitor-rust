...
3. **`use_params`/`use_params_map` panic outside a matched route** ("Tried to access params outside the context of a matched <Route>.") — never call them in the `fallback` **nor inside `ProtectedRoute`'s `condition`/`redirect_path` closures** (they run inside a `Transition` child scope where the matched-route contexts are not provided — `use_params` AND `use_location` both panic there, confirmed in `client-v2/src/views/sedes.rs`). Use `use_url()` there — it falls back to the RouterContext URL, works anywhere under the Router, and `url.get().path()` is the route's path whenever the guard runs.
