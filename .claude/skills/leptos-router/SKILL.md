...
3. **`use_params`/`use_params_map` panic outside a matched route** ("Tried to access params outside the context of a matched <Route>.") — never call them in the `fallback` **nor inside `ProtectedRoute`'s `condition`/`redirect_path` closures** (they run inside a `Transition` child scope where the params context is not provided — confirmed by panic in `client-v2/src/views/sedes.rs`). Use `use_location()` there and parse the pathname segments instead; the URL is the route's path whenever the guard runs.
